<?php
$pageTitle = 'View Customer Purchase Order';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_status'])) {
    if (!validateCsrfToken()) {
        ...
        if (in_array($newStatus, ['pending', 'approved', 'processing', 'completed', 'cancelled'])) {
            $uStmt = $conn->prepare("UPDATE customer_orders SET status = ? WHERE id = ?");
            $uStmt->bind_param("si", $newStatus, $id);
            if ($uStmt->execute()) {
                $po['status'] = $newStatus;
                $success = 'Order status updated successfully to ' . ucfirst($newStatus) . '.';
            } else {
...
// Fetch line items for this Customer Purchase Order
$itemsStmt = $conn->prepare("SELECT * FROM customer_order_items WHERE customer_order_id = ? ORDER BY id ASC");
...
<div class="d-flex flex-wrap justify-content-between align-items-center gap-3 mb-4">
    <div>
        <h2 class="mb-1"><i class="ph-bold ph-receipt text-primary"></i> Customer PO: <?php echo htmlspecialchars($po['po_number']); ?></h2>
        <span class="text-muted small">Created on <?php echo date('F d, Y', strtotime($po['order_date'])); ?></span>
    </div>
    ...
    <div class="col-lg-8">
        <div class="card shadow-sm mb-4">
            <div class="card-header bg-white py-3 d-flex justify-content-between align-items-center">
                <h5 class="mb-0 fw-semibold text-dark"><i class="ph-bold ph-info text-primary me-2"></i>Purchase Order Details</h5>
                <code><?php echo htmlspecialchars($po['po_number']); ?></code>
            </div>
            <div class="card-body p-4">
                <div class="row g-3">
                    <div class="col-sm-6">
                        <span class="text-muted small d-block">Customer / Client:</span>
                        <div class="fw-bold fs-5 text-dark"><?php echo htmlspecialchars($po['customer_name']); ?></div>
                    </div>
                    <div class="col-sm-6">
                    ...
                    <div class="col-sm-6">
                        ...
                        <div class="text-success fw-bold fs-4">₱<?php echo number_format($po['total_amount'], 2); ?></div>
                    </div>
                    <?php if (!empty($po['notes'])): ?>
                    <div class="col-12">
                        <hr class="my-2 text-muted">
                        <span class="text-muted small d-block mb-1">Notes / Instructions:</span>
